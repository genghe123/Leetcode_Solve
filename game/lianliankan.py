...
def getGameWindow():
    ...
    while not window:
        print('Failed to locate the game window')
        time.sleep(10)
        window = win32gui.FindWindow(None,WINDOW_TITLE)
    
    win32gui.SetForegroundWindow(window)
    pos = win32gui.GetWindowRect(window)
    print('Game windowss at '+ str(pos))
    return (pos[0],pos[1])
...
def getAllSquare(screen_image, game_pos):
    ...
    for one_square in all_squares:
        s = one_square[SUB_LT_Y:SUB_RB_Y,SUB_LT_X:SUB_RB_X]
        finalresult.append(s)
        cv2.imwrite("D:/Downloads/{num}.jpg".format(num=i), s)
        i += 1
...
def canConnect(x1, y1, x2, y2, r):
    ...
    if result[x1][y1] == EMPTY_ID or result[x2][y2] == EMPTY_ID:
        return False
    if (x1==x2 and y1==y2) or result[x1][y1] != result[x2][y2]:
        return False
    return horizontalCheck(x1,y1,x2,y2,result) or verticalCheck(x1,y1,x2,y2,result) or turnOnceCheck(x1,y2,x2,y2,result) or turnTwiceCheck(x1,y1,x2,y2,result)
...
def turnOnceCheck(x1,y1,x2,y2,result):
    ...
    if result[cx][cy] == EMPTY_ID and horizontalCheck(x1,y1,cx,cy,result) and verticalCheck(x2,y2,cx,cy,result):
        return True
    if result[dx][dy] == EMPTY_ID and horizontalCheck(x2,y2,dx,dy,result) and verticalCheck(x1,y1,dx,dy,result):
        return True
    return False
...
def turnTwiceCheck(x1,y1,x2,y2,result):
    ...
    for i in range(0,len(result)):
        for j in range(0,len(result[0])):
            if result[i][j] != EMPTY_ID or (i!=x1 and i!=x2 and j!=y1 and j!=y2) or (i==x1 and j == y2) or (i==x2 or j==y1):
                continue
            if turnOnceCheck(x1,y1,i,j,result) and (horizontalCheck(x2,y2,i,j,result) or verticalCheck(x2,y2,i,j,result)):
                return True
            if turnOnceCheck(x2,y2,i,j,result) and (horizontalCheck(x1,y1,i,j,result) or verticalCheck(x1,y1,i,j,result)):
                return True 
    return False
                    ...
                    for n in range(0,len(result[0])):
                        if result[m][n] != EMPTY_ID:
                            if canConnect(i,j,m,n,result):
                                ...
                                self.simulateMouseClick(x1+15,y1+18)
                                time.sleep(random.uniform(TIME_INTERVAL_MIN,TIME_INTERVAL_MAX))
                                slef.simulateMouseClick(x2+15,y2+18)
                                time.sleep(random.uniform(TIME_INTERVAL_MIN,TIME_INTERVAL_MAX))
...
def autoRemove(squares, game_pos):
    ...
    i=0
    while True:
        if autoRelease(squares, game_x, game_y):
            i += 1
        else:
            return i
